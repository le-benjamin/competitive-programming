...
int main() {
    ...
    seq.assign(pow(2, n), 0);
    read(seq);
    
    tree.resize(n+1);
    FOR(i, n, -1, -1)
        tree[n-i].resize(pow(2, i), 0);
    tree[0] = seq;

    FOR(i, n) { // layer
        FOR(j, 0, pow(2, n-i), 2) // step
            tree[i+1][j/2] = (xr ? tree[i][j]^tree[i][j+1] : tree[i][j]|tree[i][j+1]);
        xr = !xr;
    }
    ...
    while (m--) {
        ...
        xr = false;
        FOR(i, n) {
            ll other = (p%2 ? p-1 : p+1);
            tree[i+1][p/2] = (xr ? tree[i][p]^tree[i][other] : tree[i][p]|tree[i][other]);
            p /= 2;
            xr = !xr;
        }
        print(tree[n][0]);
