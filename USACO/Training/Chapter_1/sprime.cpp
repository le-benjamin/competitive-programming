...
void sieve() {
    vt<bool> primesBS(10001, true); // sqrt 10^8 = 10^4
    primesBS[0] = primesBS[1] = false;
    primes.pb(2);
    for (ll i = 3; i <= 10000; i+=2) { 
        if (!primesBS[i]) continue;
        primes.pb(i);
...
bool isPrime(int n) { // faster than bitset access bc sieve less nums & maybe something weird with random acc iterators
    EACH(x, primes) {
...
void gen(int n, int l) {
    if (!isPrime(n)) return;
    if (l == len) {
        print(n); // guaranteed to go least to greatest
    }
    else {
        n*=10; l++;
