...
int main() {
    ...
    vt<vt<int>> mat(3, vt<int>(3, 0));

    FOR(j, 3) 
        for (int i = lower_bound(all(sorted), j+1)-sorted.begin(); sorted.begin()+i < upper_bound(all(sorted), j+1); i++) 
            mat[j][seq[i]-1]++;

    int count = max(mat[0][1]+mat[0][2]+mat[1][2],
