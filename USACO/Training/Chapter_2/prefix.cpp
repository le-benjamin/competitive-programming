...
int solve() {
    vt<bool> possible(sz(S)+1, 0); 
    possible[0] = 1;
    int maxLen = 0;
    FOR(i, 1, sz(S)+1) {
        for (int j = 0; j < sz(P) && sz(P[j]) <= i; j++) {
            if (P[j] == S.substr(i-sz(P[j]), sz(P[j])) && possible[i-sz(P[j])]) {
                possible[i] = 1;
                maxLen = i;    
            }
        }
    ...
    setIO("prefix");
{
    ...
    sort(all(P), [](const auto& a, const auto& b){ return (sz(a) == sz(b) ? a < b : sz(a) < sz(b)); });
    int len = solve();
    print(len);
