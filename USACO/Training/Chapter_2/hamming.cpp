...
// helper function for getting hamming distances
int hamDist(const int& a, const int& b) {
...
int main() {
    ...
    FOR(i, (int)pow(2, B+1)-1, -1, -1) {
        out = {};
        out.pb(i);
        FOR(j, i+1, (int)pow(2, B+1)) {
            if (sz(out) == N) {
                codewords = out;
                break;
            }
