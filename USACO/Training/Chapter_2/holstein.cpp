...
vt<int> reqs;
vt<vt<int>> feeds;
vt<int> feedSet(MAX_V, MOD);
vt<int> out;
// so many vectors aahhh


bool isEnough(const vt<int>& a) {
    FOR(i, V) {
        int sum = 0;
        EACH(x, a) 
            sum += feeds[x][i];
        if (sum < reqs[i]) return false;
    }
...
void search(int start) {
    out.pb(start);
    if (isEnough(out)) {
        if (sz(out) < sz(feedSet)) feedSet = out;   
        out.pop_back();
        return;
    }
    FOR(i, start+1, G) 
