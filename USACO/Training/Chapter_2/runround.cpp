...
bool isRunaround(string num) {
    map<char, int> digits;
    // check if digits repeat
    EACH(c, num) {
        digits[c]++;
        if (c == '0' || digits[c] > 1) return false;
    }

    vt<bool> visited(sz(num), 0);
    int i = 0;
    for (i = 0; find(all(visited), 0) != visited.end(); i+=stoi(to_string(num[i])), i%=sz(num)) {
