...
int f;
vt<list<int>> adjlist(505, list<int>());
list<int> res;

void solve(int node) {

    if (sz(adjlist[node])) {
        while (sz(adjlist[node])) {
            int nx = adjlist[node].front(); 
            adjlist[node].pop_front();
            adjlist[nx].erase(find(all(adjlist[nx]), node));
            solve(nx);
        }
    } 
...
int main() {
    ...
    EACH(x, adjlist) x.sort();

    // solve on first node with odd degree, if none solve first node with non-zero degree
    int start;
    for (start = 1; start < sz(adjlist); start++) 
        if (sz(adjlist[start])%2 == 1) goto output;
    for (start = 1; start < sz(adjlist); start++) 
        if (sz(adjlist[start])) goto output;
